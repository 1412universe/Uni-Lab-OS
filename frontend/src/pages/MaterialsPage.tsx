...
import { capacityText, capacityValue, parseMaximum } from '../lib/capacity'
...
export function MaterialsPage({
  ...
  async function saveCapacity() {
    if (!connected || !detail) return
    setCapacitySaving(true)
    try {
      await updateMaterialCapacity(detail, parseMaximum(capacityForm.maximum, capacityForm.unit))
  ...
  return (
    <div className="page materials-page">
      <div className="materials-layout scene-layout">
        ...
        <Panel className="material-table-panel">
          ...
          {detail ? <div className="material-detail-workspace" aria-label="物料关系详情">
            ...
            {templatesQuery.data?.find((template) => template.uuid === detail.resourceTemplateUuid)?.tags?.includes('container') ? <section className="capacity-section"><strong>最大装料量：{capacityText(containerReagent?.maximumCapacity ?? detail.capacity, containerReagent?.quantityUnit || (detail.capacity?.max_mass_g != null ? 'g' : 'mL'))}</strong><Button disabled={!connected} onClick={() => { const unit = ['mg', 'g', 'kg'].includes(containerReagent?.quantityUnit || '') || (!containerReagent && detail.capacity?.max_mass_g != null) ? 'g' : 'mL'; setCapacityForm({ maximum: capacityValue(containerReagent?.maximumCapacity ?? detail.capacity, unit), unit }); setCapacityError(''); setDialog('capacity') }}>设置最大装料量</Button></section> : null}
      ...
      {dialog ? <div className="dialog-backdrop" role="presentation"><div className="material-write-dialog" role="dialog" aria-modal="true">
        ...
        {dialog === 'capacity' && detail ? <div className="dialog-content"><strong>{detail.name}</strong><label className="form-field"><span>计量单位</span><select aria-label="最大装料量单位" value={capacityForm.unit} onChange={(event) => { setCapacityForm({ maximum: capacityValue(containerReagent?.maximumCapacity ?? detail.capacity, event.target.value), unit: event.target.value }); setCapacityError('') }}><option value="mL">mL</option><option value="g">g</option></select></label><CapacityField value={capacityForm.maximum} unit={capacityForm.unit} defaultValue={capacityValue(detail.ratedCapacity, capacityForm.unit)} onChange={(maximum) => { setCapacityForm({ ...capacityForm, maximum }); setCapacityError('') }} />{capacityError ? <p className="form-error" role="alert">{capacityError}</p> : null}<Button tone="primary" disabled={!connected || capacitySaving} onClick={() => void saveCapacity()}>{capacitySaving ? '正在保存…' : '保存'}</Button></div> : null}
