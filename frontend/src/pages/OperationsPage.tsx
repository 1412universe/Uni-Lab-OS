...
interface DraftControl { id: string; nodeUuid: string; templateUuid: string; name: string; description?: string; param: Record<string, unknown>; parameterSchema: Record<string, unknown> }
...
export function OperationsPage({ materials: inputMaterials, connected, onNotify }: { materials: MaterialRecord[]; connected: boolean; onNotify: (message: string) => void }) {
  ...
  const [controlParamDrafts, setControlParamDrafts] = useState<Record<string, string>>({})
  const [invalidControlParamIds, setInvalidControlParamIds] = useState<Set<string>>(new Set())
  ...
  const controlParameterProblem = invalidControlParamIds.size > 0 || draftControls.some((control) => !control.param || typeof control.param !== 'object' || Array.isArray(control.param))
  ...
  const workflowContractProblem = missingWorkflowContract
    ...
    : incompatibleWorkflowContract
      ? '工作流参数类型与节点输入类型不兼容'
      : ''
  const saveProblem = !name.trim() ? '请填写操作名称' : !draft.length && !draftControls.length && !childWorkflowRefs.length ? '请从左侧加入至少一个 Action，或引用一个已发布子工作流' : missingBinding ? '存在未绑定设备实例的步骤' : missingChildBinding ? '子工作流存在未绑定的执行设备' : missingParameter ? '存在未配置的必填节点参数' : controlParameterProblem ? '控制节点参数必须是 JSON 对象' : workflowContractProblem || contractProblem
  ...
  function beginCreate() { setCreating(true); setEditingUuid(''); setSelectedOperation(null); setName(''); setDescription(''); setCategoryUuid(''); setInputContractFields([]); setOutputContractFields([]); setChildWorkflowRefs([]); setChildPickerOpen(false); setDraft([]); setDraftControls([]); setInvalidControlParamIds(new Set()); setExpandedControlId('') }
  ...
  async function beginEdit(operation: WorkflowDefinition) {
    try {
      ...
      const controls: DraftControl[] = graph.nodes.filter((node) => node.workflow_node_template_uuid && !node.meta_data?.unilab?.composite && isControlNode(node)).map((node) => {
        ...
        return {
          id: crypto.randomUUID(), nodeUuid: String(node.uuid), templateUuid: String(node.workflow_node_template_uuid),
  ...
  async function submit() {
    ...
    try {
      ...
      let savedRevision = 1
      if (editingUuid) {
        await updateExperimentOperation({ workflowUuid: editingUuid, name: name.trim(), description: description.trim(), categoryUuid: categoryUuid || undefined, inputContract, outputContract, actions: draft.map(({ nodeUuid, templateUuid, materialUuid, deviceId, name: actionName, description: actionDescription, param, inputBindings }) => ({ nodeUuid, templateUuid, materialUuid, deviceId, name: actionName, description: actionDescription, param, inputBindings })), controls: draftControls.map(({ nodeUuid, templateUuid, name: controlName, description: controlDescription, param }) => ({ nodeUuid, templateUuid, name: controlName, description: controlDescription, param })) })
  ...
  /** 解析控制节点 JSON；非法值不覆盖旧参数并阻止保存。 */
  function commitControlParam(control: { id: string; name: string }, raw: string) {
    try {
      const value = JSON.parse(raw)
      if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('object')
      setDraftControls((current) => current.map((item) => item.id === control.id ? { ...item, param: value } : item))
      setInvalidControlParamIds((current) => { const next = new Set(current); next.delete(control.id); return next })
      onNotify(`已更新控制节点“${control.name}”参数`)
    } catch {
      setInvalidControlParamIds((current) => new Set(current).add(control.id))
      onNotify(`控制节点“${control.name}”参数必须是有效 JSON 对象`)
    }
  }
 ...
 return <div className="page operations-page">
    ...
    <div className={`operation-authoring-layout ${creating ? 'is-creating' : ''}`}>
      <Panel className="device-action-browser"><PanelHeader title="设备模板" description={`${deviceGroups.length} 个设备 · ${actionsQuery.data?.length || 0} 个 Action`} />
        ...
        <section className="device-actions control-template-list"><header><strong>工作流控制节点</strong><span>{controlTemplatesQuery.data?.length || 0} 个</span></header>{controlTemplatesQuery.data?.map((control) => <div className="control-template-card" key={control.uuid}><div><strong>{control.displayName}</strong><code>{control.nodeType}</code><small>{control.description || '调度器在运行时处理，不占用设备执行器。'}</small></div><span>从 JSON / Python 导入后可编辑</span></div>)}{!controlTemplatesQuery.data?.length ? <small className="control-template-empty">当前设备包未提供条件或循环节点。</small> : null}</section>
      </Panel>
      {creating ? <Panel className="operation-builder"><div className="builder-heading"><div><span>EXPERIMENT OPERATION EDITOR</span><h2>{editingUuid ? '编辑实验操作' : '创建实验操作'}</h2>{editingUuid ? <code>{editingUuid}</code> : null}<p>{editingUuid ? '保存修改后状态退回 source，需要重新发布。' : '选择设备 Action 或引用已发布实验操作，组成可复用子工作流。条件、循环节点请通过 JSON / Python 导入。'}</p></div><div className="builder-heading-actions"><strong>{draft.length + draftControls.length + childWorkflowRefs.length} 个节点</strong><Button tone="primary" icon={<Save size={15} />} disabled={saving} title={saveProblem || undefined} onClick={() => void submit()}>{saving ? '正在保存…' : '保存实验操作'}</Button></div></div>
