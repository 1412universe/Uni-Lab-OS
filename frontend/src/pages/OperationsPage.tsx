...
import { ArrowDown, ChevronLeft, ChevronRight, Eye, FlaskConical, GripVertical, Pencil, Plus, Save, Search, Send, Sparkles, Trash2, Workflow } from 'lucide-react'
  ...
  return <div className="page operations-page">
    ...
    <div className={`operation-authoring-layout ${creating ? 'is-creating' : ''}`}>
      ...
      {creating ? <Panel className="operation-builder"><div className="builder-heading"><div><span>EXPERIMENT OPERATION EDITOR</span><h2>{editingUuid ? '编辑实验操作' : '创建实验操作'}</h2>{editingUuid ? <code>{editingUuid}</code> : null}<p>{editingUuid ? '保存修改后状态退回 source，需要重新发布。' : '选择设备 Action 形成顺序节点；保存后发布为可复用子工作流。'}</p></div><div className="builder-heading-actions"><strong>{draft.length} 个节点</strong><Button tone="primary" icon={<Save size={15} />} disabled={Boolean(saveProblem) || saving} title={saveProblem} onClick={() => void submit()}>{saving ? '正在保存…' : '保存实验操作'}</Button></div></div>
        ...
        <div className="operation-sequence">{draft.map((item, index) => { const template = actionsQuery.data?.find((action) => action.uuid === item.templateUuid); const devices = materials.filter((material) => material.resourceTemplateUuid === template?.resourceTemplate.uuid && material.sourceNodeId); return <div className="operation-step-card" key={item.id}><div className="operation-step"><span className="step-number">{String(index + 1).padStart(2, '0')}</span><GripVertical size={15} /><div><strong>{item.name}</strong><small>{template?.resourceTemplate.displayName} · {item.fields.length} 个参数</small></div><select value={item.materialUuid} onChange={(event) => { const device = devices.find((candidate) => candidate.uuid === event.target.value); setDraft((current) => current.map((action) => action.id === item.id ? { ...action, materialUuid: device?.uuid || '', deviceId: device?.sourceNodeId || '' } : action)) }}><option value="">选择设备实例</option>{devices.map((device) => <option value={device.uuid} key={device.uuid}>{device.name}</option>)}</select><div className="step-controls"><button onClick={() => setExpandedActionId((current) => current === item.id ? '' : item.id)}>参数</button><button disabled={index === 0} onClick={() => moveAction(index, -1)}>↑</button><button disabled={index === draft.length - 1} onClick={() => moveAction(index, 1)}>↓</button><button onClick={() => setDraft((current) => current.filter((action) => action.id !== item.id))}>×</button></div>{index < draft.length - 1 ? <span className="sequence-edge"><ArrowDown size={13} />ready</span> : null}</div>{expandedActionId === item.id ? <div className="node-parameter-editor"><header><strong>配置节点参数</strong><span>固定值或绑定工作流参数</span></header>{item.fields.length ? item.fields.map((field) => { const binding = item.inputBindings[field.handleUuid]; const source = binding ? 'workflow' : 'literal'; return <div className="node-parameter-row" key={field.handleUuid}><label><span>{field.displayName}{field.required ? ' *' : ''}</span><code>{field.key}</code></label><select value={source} onChange={(event) => setDraft((current) => current.map((action) => { if (action.id !== item.id) return action; const nextBindings = { ...action.inputBindings }; if (event.target.value === 'workflow') nextBindings[field.handleUuid] = { parameter: field.key }; else delete nextBindings[field.handleUuid]; return { ...action, inputBindings: nextBindings } }))}><option value="literal">固定值</option><option value="workflow">工作流参数</option></select>{source === 'workflow' ? <input aria-label={`工作流参数 ${field.key}`} value={binding?.parameter || ''} onChange={(event) => setDraft((current) => current.map((action) => action.id === item.id ? { ...action, inputBindings: { ...action.inputBindings, [field.handleUuid]: { parameter: event.target.value } } } : action))} placeholder="参数名称" /> : <input aria-label={`节点参数 ${field.key}`} value={actionValue(item.param[field.key])} onChange={(event) => { try { const value = parseParameterValue(event.target.value, field.schema); setDraft((current) => current.map((action) => action.id === item.id ? { ...action, param: { ...action.param, [field.key]: value } } : action)) } catch { /* 保留上一个有效 JSON */ } }} placeholder={String(field.schema.type || field.schema.$slot || 'string')} />}</div> }) : <EmptyState title="该 Action 没有输入参数" description="输出端口和 ready 控制依赖由模板定义。" />}</div> : null}</div> })}{!draft.length ? <EmptyState title="尚未添加工作流节点" description="从左侧选择设备，再点击设备下的 Action 加入实验操作。" /> : null}</div>
