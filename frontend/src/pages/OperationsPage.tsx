...
export function OperationsPage({ materials: inputMaterials, connected, onNotify }: { materials: MaterialRecord[]; connected: boolean; onNotify: (message: string) => void }) {
  ...
  function beginCreate() {
    if (!connected) return
    setCreating(true); setEditingUuid(''); setSelectedOperation(null); setName(''); setDescription(''); setCategoryUuid(''); setInputContractFields([]); setOutputContractFields([]); setChildWorkflowRefs([]); setChildPickerOpen(false); setDraft([]); setDraftControls([]); setExpandedControlId('')
  }
