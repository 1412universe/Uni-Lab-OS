...
工作流和实验操作使用同一编译模型，但用途不同：普通 `normal` 工作流可以创建 Task；`experiment_operation` 必须先发布，再作为上层流程的子工作流使用，不能在产品模式下独立当作完整实验运行。
...
输出可以用一个 `TypedDict` 声明正式合同，也可以使用 `workflow_output(...)`：
...
每个动作、条件、循环、展示分组和子工作流调用前都要有紧邻的唯一 UUID：
...
节点 UUID 用于画布 round-trip、修订差异、Task 矩阵、Job 归属和恢复，不能因格式化代码随意改变。可选标题必须在 UUID 注释正上方并保持相同缩进。
...
1. 按[用 AI 编写工作流（推荐）](ai-workflow-authoring.md)建立取证、生成、审查和验证闭环；
2. 按[手写并运行第一个工作流](first-workflow.md)掌握最小 DSL，具备人工审查和排障能力；
3. 在[工作流编排特性](workflow-features.md)学习条件、循环、并行、资源、物料、人工确认和子工作流；
4. 在[工作流](workflows.md)掌握发布、预检和版本边界；
5. 在[任务与并行调度](tasks.md)理解任务运行与恢复。
