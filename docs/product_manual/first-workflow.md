...
SZLab 控制流探针在设备元数据中声明 `hardware_side_effects=False`。本地环境仍应保持[安装教程](installation.md)启动的 `dry-run + develop`，这样即使误选其他动作，也不会构造真机驱动。

## 完成前检查

- Workspace Backend 与 Edge 均为 `ready`；
- Console 的“设备”页能找到 `szlab_control_flow_probe`；
- “工作流”页能看到 SZLab 已登记流程；
- 当前启动模式为 `develop`，运行模式为 `dry-run`。

如果还没有本地环境，先完成[安装并启动本地产品](installation.md)。
...
from szlab_poly_studio.devices.control_flow_probe.device import (
    SZLabControlFlowProbeDevice,
)
...
2. 设备类型来自真实 SZLab 驱动包；编译器用它解析动作签名。
3. `device("szlab_control_flow_probe")` 固定绑定 Graph 中的无硬件副作用设备。
...
如果页面显示模板或设备未绑定，不要继续发布。返回安装检查，确认 SZLab 包、Graph 与 Edge 设备目录属于同一代 Workspace。
...
如果是 `temporarily_unavailable`，先确认 Edge 已连接且软件探针没有被其他任务占用。如果是 `invalid`，回到诊断、输入与发布状态逐项修复。
...
在 `dry-run` 中，模拟器会按动作结果合同生成确定性回执：同名输出优先透传输入，所以 `observed` 应与提交的 `value` 相同。字符串 `message` 可以是模拟默认值；它不是 SZLab 真驱动生成的中文消息。
...
- Python DSL 能被静态编译；
- 发布合同可运行；
- Scheduler 能创建并派发 Job；
- Edge 模拟动作能通过正常结果通道回传；
- 工作流输出能从动作结果投影。

## 8. 改用 API 完成同一闭环（可选）

UI 是推荐入口；下面的 API 是尚未执行 UI 导入时的等价入口，不要在已导入同一 UUID 后重复调用。先把 `BACKEND_URL` 改为状态命令返回的 Backend 地址；如果你已经完成前面的 UI 教程，请先为工作流和节点换一组新 UUID。
...
:::{note}
Python 导入是“创建新定义”，不是覆盖已有 UUID。需要长期迭代时，把源码放在领域包的 `workflows/` 目录并登记到 `package.yaml`，或使用 Authoring 的 draft → candidate → apply 并发控制接口。不要反复上传同一个 UUID 期待覆盖。
:::
