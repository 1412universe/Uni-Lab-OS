...
本教程用不触发工艺设备动作的“控制流验收 1：条件分支”完成一次提交、调度和结果查看。它适合确认 Uni-Lab OS 的基本运行链路，但不能代替真机安全验收。
...
- 你已获准使用当前演示环境。
- 你已阅读[使用 Uni-Lab OS](console.md)，知道怎样识别连接、模式和只读快照。
- Uni-Lab OS 页面显示系统在线且设备连接正常。
- 当前模式可以保持 `develop`；这个教程不要求切换到 `product`。
- “工作流”页面中可以找到已发布的“控制流验收 1：条件分支”。

## 1. 检查连接

运行 `unilab workspace status --json` 取得 Uni-Lab OS 页面地址并打开。先在“总监控”页检查：
...
如果任务带有 Trace，点击任务详情中的 Trace 链接打开追踪页面。Trace 用于关联 Uni-Lab OS 内部的调度与设备执行过程；设备包动作日志中的短 `trace` 标记是局部日志标识，不应相互替代。

## 下一步

- 想自己写一个新流程，先读[工作流基本模型](workflow-concepts.md)，再做[用户设备包 手写教程](first-workflow.md)。
...
- 需要运行 用户设备包 工艺时，按[用户设备包 场景指南](scenario-guide.md)核对输入、设备边界和输出含义。
- 需要观察设备 Driver 与 OPC UA 握手时，先按[PLC / OPC UA 仿真器](plc-sim.md)检查 Server、可选握手代理和 Uni-Lab OS 设备连接。
