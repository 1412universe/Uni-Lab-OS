...
WORKFLOW_TYPE_SUBWORKFLOW = "subworkflow"
WORKFLOW_TYPES = frozenset(
    {
        WORKFLOW_TYPE_NORMAL,
        WORKFLOW_TYPE_SUBWORKFLOW,
    }
)

WorkflowType = Literal["normal", "subworkflow"]
...
def normalize_workflow_type(
    ...
    *,
    ...
    兼容尚未声明类型的旧工作流。返回：``normal`` 或 ``subworkflow``。异常：
    非字符串或未知字符串抛出 ``ValueError``，禁止把拼写错误静默归为普通流程。
    """

    candidate = default if value is None else value
    if not isinstance(candidate, str) or candidate not in WORKFLOW_TYPES:
        raise ValueError("workflow_type must be normal or subworkflow")
